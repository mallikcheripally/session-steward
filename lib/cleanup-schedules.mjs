...
async function findCandidates(schedule, protectionStore, provider, settings, now) {
  const options = listingOptions(schedule, settings, now);
  const first = await provider.listSessions({ ...options, page: 1 });
  const snapshot = protectionStore ? await protectionStore.list() : null;
  const providerHome = schedule.providerHomeOverride ?? settings.getHome(schedule.provider);
  const records = [];
  let protectedSkippedCount = 0;
  const startPage = options.sort === "size" ? 1 : first.pageCount;
  const endPage = options.sort === "size" ? first.pageCount : 1;
  const step = options.sort === "size" ? 1 : -1;

  for (let page = startPage; step > 0 ? page <= endPage : page >= endPage; page += step) {
    const result = page === 1 ? first : await provider.listSessions({ ...options, page });
    const pageRecords = options.sort === "size" ? result.records : [...result.records].reverse();
    for (const record of pageRecords) {
      const kept = protectionStore?.classify({
        providerHome,
        providerId: schedule.provider,
        record,
        snapshot,
      }).kept;
      if (kept) protectedSkippedCount += 1;
      else records.push(record);
      if (records.length >= schedule.maxSessions) {
        return { protectedSkippedCount, records };
      }
    }
  }
  return { protectedSkippedCount, records };
...
function safeRunResult(result, candidateCount, protectedSkippedCount, atMs) {
  return {
    ...
    protectedSkippedCount: protectedSkippedCount + (result.skippedProtectionCount ?? 0),
...
export async function runCleanupSchedule({
  ...
  try {
    ...
    const selection = await findCandidates(schedule, protectionStore, provider, settings, now);
    ...
    if (candidates.length === 0) {
      run = {
        ...
        protectedSkippedCount: selection.protectedSkippedCount,
        status: selection.protectedSkippedCount > 0 ? "no-unprotected-matches" : "no-matches",
        transcriptBytes: 0,
      };
    } else {
      ...
      run = safeRunResult(result, candidates.length, selection.protectedSkippedCount, atMs);
    }
  } catch {
    run = {
      affectedSessionCount: 0,
