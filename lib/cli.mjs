...
Commands
  ...
  keep <selector> [...]         Keep sessions from Session Steward cleanup
  unkeep <selector> [...]       Remove a session Keep
  keep-workspace <path>         Keep a workspace and its descendants
  unkeep-workspace <path>       Remove a workspace Keep
...
function getMarkerText(record) {
  ...
  if (record.keep?.kept) {
    markers.push(record.keep.workspace ? "kept:workspace" : "kept");
  }
...
function printScreen({ archiveStatus, cleanupMode, inactiveDays, providerHome, providerName, result, search, showInternals, showSupporting, showTokens, sort, workspace }) {
  ...
  output.write(
    "Commands: search | workspace | inactive | archive | sort | inspect | keep | unkeep | delete | overview | backups | help | quit\n\n",
  );
...
async function refreshPage(state) {
  ...
  const snapshot = await state.protectionStore.list();
  state.result.records = state.result.records.map((record) => ({
    ...record,
    keep: state.protectionStore.classify({
...
async function printJson(options) {
  ...
  let written = 0;
  let first = true;
  const protectionSnapshot = await options.protectionStore.list();
  await writeChunk("[\n");
  ...
  while (written < limit) {
    ...
    for (const record of result.records) {
      ...
      serializedRecord.keep = options.protectionStore.classify({
...
async function runInteractive(state) {
  ...
  try {
    while (true) {
      ...
      if (["keep", "unkeep"].includes(command.name)) {
        try {
          const sessionIds = parseSelectors(command.args, state.result.records);
          if (sessionIds.length === 0) throw new Error("Choose at least one session.");
          const input = {
            providerHome: state.providerHome,
            providerId: state.provider.id,
            sessionIds,
          };
          if (command.name === "keep") await state.protectionStore.keepSessions(input);
          else await state.protectionStore.removeSessions(input);
          output.write(`\n${command.name === "keep" ? "Kept" : "Stopped keeping"} ${sessionIds.length} ${sessionIds.length === 1 ? "session" : "sessions"}. This changes only Session Steward cleanup.\n`);
          state.forceRefresh = true;
        } catch (error) {
          output.write(`\n${error instanceof Error ? error.message : String(error)}\n`);
          await pause(rl);
      ...
      if (["keep-workspace", "unkeep-workspace"].includes(command.name)) {
        try {
          const workspacePath = command.args.join(" ") || state.workspace;
          if (!workspacePath) throw new Error("Enter a workspace path or set the workspace filter first.");
          if (command.name === "keep-workspace") {
            await state.protectionStore.keepWorkspace({ workspace: workspacePath });
          } else {
            await state.protectionStore.removeWorkspace({ workspace: workspacePath });
          }
          output.write(`\n${command.name === "keep-workspace" ? "Kept" : "Stopped keeping"} workspace ${workspacePath}. This changes only Session Steward cleanup.\n`);
          state.forceRefresh = true;
        } catch (error) {
          output.write(`\n${error instanceof Error ? error.message : String(error)}\n`);
          await pause(rl);
      ...
      if (command.name === "delete") {
        try {
          ...
          const prepared = await prepareSessionCleanup({
            options,
            protectionStore: state.protectionStore,
            provider: state.provider,
            recordIds: sessionIds,
            scope,
          });
          ...
          if (prepared.skippedProtections.length > 0) {
            output.write(`Skipped ${prepared.skippedProtections.length} kept ${prepared.skippedProtections.length === 1 ? "selection" : "selections"}.\n`);
          }
          if (deletionPlan.ids.length === 0) {
            output.write("Nothing will be deleted. Keep affects only Session Steward cleanup.\n");
            await pause(rl);
            continue;
          }
...
export async function runCli(options) {
  ...
  const protectionStore = options.protectionStore ?? createSessionProtectionStore({
  ...
  if (options.json) {
    await printJson({ ...options, protectionStore, provider, providerHome });
    return;
  }
