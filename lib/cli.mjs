...
function printScreen({ archiveStatus, inactiveDays, providerHome, providerName, result, search, showInternals, sort, workspace }) {
  if (output.isTTY) {
    output.write("\x1Bc");
  }
  ...
  output.write(
    "Commands: search | workspace | inactive | archive | sort | inspect | delete | page | next | prev | internals | refresh | help | quit\n\n",
  );
  ...
  try {
    while (true) {
      ...
      if (command.name === "delete") {
        try {
          const sessionIds = parseSelectors(command.args, state.result.records);
          await state.provider.assertDeepCleanupSupported(providerOptions(state.provider.id, state.providerHome));
          const deletionStore = await state.provider.loadDeletionStore({
            ...providerOptions(state.provider.id, state.providerHome),
            recordIds: sessionIds,
          });
          ...
          await state.provider.preflightSessionDeletion({
            plan: deletionPlan,
            scope: "deep",
            store: deletionStore,
          });
...
export async function runCli(options) {
  ...
  if (options.json) {
    await printJson({ ...options, provider });
    return;
  }
  ...
  const state = {
    ...
    provider,
    providerHome: path.resolve(options.providerHome),
