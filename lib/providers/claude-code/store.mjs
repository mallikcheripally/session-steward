...
import { claudeProcessState, localClaudePidDomain } from "./process-state.mjs";
...
// Claude's project-level siblings. Do not use a broad session-ID prefix:
// unknown suffixes and another session's files must remain untouched.
function indexSessionSidecars(directory, entries) {
  const byId = new Map();
  for (const entry of entries) {
    const id = /^(.*)\.(?:ccr-tip|precompact|desktop-released|dir-sync)\.json$/u.exec(entry.name)?.[1]
      ?? /^(.*)-[0-9]{1,16}\.cast$/u.exec(entry.name)?.[1];
    if (!id) continue;
...
function sidecarsForTranscript(index, transcriptPath, id) {
  // Conflicting transcript filenames/content do not establish ownership.
  return path.basename(transcriptPath) === `${id}.jsonl` ? index.get(id) ?? [] : [];
}
...
async function discover(claudeHome, desktopDataHome) {
  ...
  for (const projectEntry of projectDirectories) {
    ...
    const sidecars = indexSessionSidecars(projectDirectory, files);
    for (const file of files) {
      ...
      summary.sidecarPaths = sidecarsForTranscript(sidecars, summary.transcriptPath, summary.id);
  ...
  for (const [id, copies] of summariesById) {
    ...
    records.push({
      ...
      sidecarPaths: copies.flatMap((item) => item.sidecarPaths),
...
function filterRecords(records, options) {
  ...
  return records.filter((record) => {
    if (typeof options.recordFilter === "function" && !options.recordFilter(record)) return false;
...
export async function getSessionOverview({ claudeHome, desktopDataHome, refresh = false }) {
  ...
  for (const record of store.records) {
    ...
    for (const target of record.sidecarPaths) {
      // Unknown directory/symlink layouts are not part of the recognized size.
      const stats = await fs.lstat(target).catch((error) => {
        if (error.code === "ENOENT") return null;
        throw error;
      });
      if (stats?.isFile()) storageTargets.add(target);
    }
...
export async function planSessionDeletion({ recordIds, store }) {
  ...
  for (const record of records) {
    ...
    for (const target of record.sidecarPaths) {
      ...
      if (!stats) continue;
      if (!stats.isFile()) throw new Error("Cleanup stopped because a Claude session sidecar is not a regular file. It was left unchanged.");
...
async function activeSessionIds(paths, selectedIds) {
  const active = new Set();
  const unverified = new Set();
  const selected = new Set(selectedIds);
  const pidDomain = await localClaudePidDomain();
  ...
  while (pending.length) {
    ...
    try { entries = await fs.readdir(current, { withFileTypes: true }); } catch (error) {
      ...
      for (const id of selected) unverified.add(id);
      continue;
    }
    for (const entry of entries) {
      const target = path.join(current, entry.name);
      if (entry.isDirectory()) { pending.push(target); continue; }
      const nameId = /([0-9a-f]{8}-[0-9a-f-]{27,})/iu.exec(entry.name)?.[1];
      const filenamePid = /^([1-9]\d*)\.json$/u.exec(entry.name)?.[1];
      if (!entry.isFile()) {
        if (selected.has(nameId)) unverified.add(nameId);
        else if (filenamePid) for (const id of selected) unverified.add(id);
        continue;
      }
      ...
      let parsed;
      try {
        // Registry metadata is small; do not read an arbitrary large file.
        const handle = await fs.open(target, "r");
        try {
          const { bytesRead } = await handle.read(buffer, 0, buffer.length, 0);
          if (bytesRead === buffer.length) throw new Error("Oversized session marker.");
          parsed = JSON.parse(buffer.toString("utf8", 0, bytesRead));
          if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error("Invalid session marker.");
        } finally { await handle.close(); }
      } catch (error) {
        if (error.code === "ENOENT") continue;
        if (selected.has(nameId)) unverified.add(nameId);
        else if (filenamePid && await claudeProcessState({ pid: Number(filenamePid) }) !== "inactive") {
          for (const id of selected) unverified.add(id);
        }
        continue;
      }
      const id = parsed.sessionId ?? parsed.session_id ?? nameId;
      if (selected.has(nameId) && id !== nameId) unverified.add(nameId);
      if (!selected.has(id)) continue;
      const pid = parsed.pid ?? (filenamePid ? Number(filenamePid) : undefined);
      if (filenamePid && parsed.pid !== undefined && parsed.pid !== Number(filenamePid)) {
        unverified.add(id);
        continue;
      }
      if (pid === undefined && ["stopped", "completed", "failed"].includes(parsed.status ?? parsed.state)) continue;
      const state = await claudeProcessState({ ...parsed, pid }, { pidDomain });
      if (state === "active") active.add(id);
      if (state === "unverified") unverified.add(id);
    }
  }
  return { active, unverified, detection: await exists(paths.sessionsDirectory) ? "available" : "unavailable" };
}

async function assertSelectedSessionsClosed(paths, ids) {
  const result = await activeSessionIds(paths, ids);
  if (result.active.size) throw new Error("Close the selected Claude sessions before cleanup.");
  if (result.unverified.size) throw new Error("Could not verify whether the selected Claude sessions are closed. Their process markers could not be checked safely; retry cleanup after Claude has exited.");
  return result;
}

export async function preflightSessionDeletion({ availableDiskBytes, plan, scope, store }) {
  const active = await assertSelectedSessionsClosed(store.paths, plan.ids);
...
// Claude's prompt-history writer uses proper-lockfile with a 10-second stale
// threshold. Use the same canonical path, directory lock and heartbeat rather
// than Session Steward's separate mutation lock (which Claude does not know).
async function lockHistory(historyPath) {
  const canonicalPath = path.join(await fs.realpath(path.dirname(historyPath)), path.basename(historyPath));
  const stats = await fs.lstat(canonicalPath).catch((error) => {
    if (error.code === "ENOENT") return null;
    throw error;
  });
  if (stats && !stats.isFile()) throw new Error("Claude prompt history is not a regular file. It was left unchanged.");
  let compromised;
  let released = false;
  const release = await lockfile.lock(canonicalPath, {
    realpath: false, // Restore also needs to lock history that does not exist yet.
    stale: 10_000,
    retries: { retries: 3, minTimeout: 50 },
    onCompromised: (error) => { compromised = error; },
  }).catch((error) => {
    if (error.code === "ELOCKED") {
      throw new Error("Claude prompt history is busy. Retry cleanup or restore in a moment.", { cause: error });
    }
    throw error;
  });
  return {
    assertHeld() {
      if (compromised) throw new Error("Claude prompt-history lock was lost. No further history changes were made.", { cause: compromised });
    },
    async release() {
      if (released) return;
      released = true;
...
async function createBackup(plan, store, scope, acquireHistoryLock) {
  ...
  try {
    ...
    const sharedJsonl = [];
    if (plan.historyMatchCount) {
      // Do not block other sessions while the large transcript copies run.
      // Keep this lock from the history backup through its replacement.
...
export async function executeSessionDeletion({ onProgress = () => {}, plan, scope, shouldCancel = () => false, store }) {
  ...
  try {
    backupDirectory = await createBackup(plan, store, scope, async () => {
      historyLock = await lockHistory(store.paths.historyPath);
    });
    ...
    await assertSelectedSessionsClosed(store.paths, plan.ids);
    historyLock?.assertHeld();
    ...
    if (plan.historyMatchCount && await exists(store.paths.historyPath)) {
      ...
      await rewriteJsonlFile(store.paths.historyPath, (entry) => !entry.parsed || !ids.has(entry.parsed.sessionId ?? entry.parsed.session_id), {
        beforeCommit: () => historyLock.assertHeld(),
      });
    }
    ...
    error.backupDirectory ??= backupDirectory;
    backupDirectory = error.backupDirectory;
...
export async function verifySessionDeletion({ plan, scope, store }) {
  ...
  // Also catch known sidecars created/recreated after the selection was planned.
  const sidecarsByDirectory = new Map();
  for (const record of plan.records) {
    for (const transcriptPath of record.transcriptPaths) {
      const directory = path.dirname(transcriptPath);
      if (!sidecarsByDirectory.has(directory)) {
        const entries = await fs.readdir(directory, { withFileTypes: true }).catch((error) => {
          if (error.code === "ENOENT") return [];
          throw error;
        });
        sidecarsByDirectory.set(directory, indexSessionSidecars(directory, entries));
      }
      for (const target of sidecarsForTranscript(sidecarsByDirectory.get(directory), transcriptPath, record.id)) {
        if (!remainingTranscriptPaths.includes(target)) remainingTranscriptPaths.push(target);
      }
    }
  }
...
export async function restoreSessionDeletionBackup({ backupDirectory, claudeHome, desktopDataHome, onProgress = () => {} }) {
  ...
  try {
    ...
    for (const entry of manifest.sharedJsonl ?? []) {
      ...
      await fs.mkdir(path.dirname(destination), { recursive: true });
      const historyLock = await lockHistory(destination);
      try {
        if (await exists(destination)) {
          const safety = path.join(safetyBackupDirectory, entry.root, entry.relative);
          await copyTarget(destination, safety);
