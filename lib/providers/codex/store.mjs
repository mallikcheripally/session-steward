...
import {
  deleteDesktopCatalogs,
  readDesktopCatalogs,
  restoreDesktopCatalogs,
  validateDesktopCatalogBackup,
} from "./desktop-catalog.mjs";
...
export async function listSessions({
  ...
  if (paths.stateDatabases.length === 1 && !needsSizeIndex && !forceUnion && typeof recordFilter !== "function") {
  ...
  const compactSizeIds = resolvedSort === "size" && paths.stateDatabases.length === 1 && typeof recordFilter !== "function";
  ...
  for (const database of paths.stateDatabases) {
    ...
    const filterSelection = typeof recordFilter === "function"
      ? `, ${stateSchema(database).columns.has("cwd") ? "coalesce(t.cwd, '')" : "''"} as filter_cwd`
      : "";
    compareSortRows ??= sessionSort?.compare ?? null;
    const rows = queryRows(
      database.path,
      `select t.id${sessionSort ? `, ${sessionSort.selection}` : ""}${filterSelection} from threads t ${conditions.sql}`,
      conditions.parameters,
    );
    for (const row of rows) {
      ...
      seenIds?.add(id);
      if (typeof recordFilter === "function" && !recordFilter({ cwd: String(row.filter_cwd || ""), id })) {
        continue;
      }
...
export async function fingerprintSessionDeletion({ plan, scope, store }) {
  ...
  hash.update(`desktop-catalogs\0${JSON.stringify(readDesktopCatalogs(store.codexHome, plan.ids))}\0`);
...
async function estimateBackupBytes({ plan, scope, store }) {
  let sourceBytes = Buffer.byteLength(JSON.stringify(readDesktopCatalogs(store.codexHome, plan.ids), null, 2));
...
export async function preflightSessionDeletion({ availableDiskBytes, plan, scope = "deep", store }) {
  // Fail on an unknown app catalogue before backing up or mutating sessions.
  readDesktopCatalogs(store.codexHome, plan.ids);
...
async function createOperationBackup({ onProgress, plan, scope, store }) {
  ...
  await atomicWriteFile(
    ...
    `${JSON.stringify({
      ...
      desktopCatalogs: readDesktopCatalogs(store.codexHome, plan.ids),
...
export async function executeSessionDeletion({
  ...
  try {
    const operation = await readJsonFile(path.join(backupDirectory, "operation.json"));
    deleteDesktopCatalogs(store.codexHome, plan.ids, operation.desktopCatalogs);
...
export async function restoreSessionDeletionBackup({
  ...
  const desktopCatalogs = operation.desktopCatalogs ?? [];
  validateDesktopCatalogBackup(codexHome, operation.ids ?? [], desktopCatalogs);
  ...
  try {
    ...
    restoreDesktopCatalogs(codexHome, operation.ids ?? [], desktopCatalogs);
...
export async function verifySessionDeletion({ plan, scope = "deep", store }) {
  const remainingDesktopCatalogEntries = readDesktopCatalogs(store.codexHome, plan.ids)
    .flatMap((entry) => entry.rows.map((row) => ({ filename: entry.filename, id: row.thread_id })));
  const deletedIdSet = new Set(plan.ids);
  ...
  return {
    complete:
      ...
      remainingDesktopCatalogEntries.length === 0 &&
      remainingDesktopStateReferences.length === 0,
    remainingDesktopCatalogEntries,
