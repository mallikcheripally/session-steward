  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "GET" && requestUrl.pathname === "/api/sessions") {
        ...
        const provider = getProvider(providerId);
        const home = settings.getHome(providerId);
        const protectionSnapshot = await protectionStore.list();
        const protectionMatcher = protectionStore.matcher({
          providerHome: home,
          providerId,
          snapshot: protectionSnapshot,
        });
        const keepFilter = requestUrl.searchParams.get("keep");
        if (keepFilter && keepFilter !== "kept") {
          throw new Error("Keep filter must be kept.");
        }
        const result = await provider.listSessions({
          ...
          workspace: requestUrl.searchParams.has("workspace")
            ...
            : undefined,
          recordFilter: keepFilter === "kept"
            ? (record) => protectionMatcher(record).kept
            : null,
        });
        sendJson(response, 200, {
          ...result,
          records: result.records.map((record) => ({
            ...provider.formatSessionForJson(record),
            keep: protectionMatcher(record),
          })),
        });
        return;
      }
      ...
      if (request.method === "GET" && requestUrl.pathname === "/api/protections") {
        const providerId = resolveProviderId(requestUrl.searchParams.get("provider"));
        const kind = requestUrl.searchParams.get("kind") || "workspaces";
        if (kind !== "workspaces") throw new Error("Keep rule kind must be workspaces.");
        const result = await protectionStore.listWorkspaceRules({
          page: getPositiveInteger(requestUrl.searchParams.get("page"), 1),
          pageSize: getPositiveInteger(requestUrl.searchParams.get("pageSize"), 25, 100),
          search: requestUrl.searchParams.get("search"),
        });
        sendJson(response, 200, { ...result, providerId });
        return;
      }

      if (["PUT", "DELETE"].includes(request.method) && requestUrl.pathname === "/api/protections") {
        requireMutationAuthorization({ request, requestUrl, token: mutationToken });
        if (activeOperationId) {
          sendJson(response, 409, { error: "Wait for the current cleanup to finish before changing Keep." });
          return;
        }
        ...
        if (body.kind === "session") {
          ...
          if (request.method === "PUT") await protectionStore.keepSession(input);
          else await protectionStore.removeSession(input);
        } else if (body.kind === "sessions") {
          const providerId = resolveProviderId(body.providerId);
          const input = {
            providerHome: settings.getHome(providerId),
            providerId,
            sessionIds: normalizeIds(body.sessionIds).map(getSessionId),
          };
          if (request.method === "PUT") await protectionStore.keepSessions(input);
          else await protectionStore.removeSessions(input);
        } else if (body.kind === "workspace") {
          if (request.method === "PUT") await protectionStore.keepWorkspace({ workspace: body.workspace });
          else await protectionStore.removeWorkspace({ workspace: body.workspace });
        } else {
          throw new Error("Keep kind must be session, sessions, or workspace.");
        }
        deletionPlans.clear();
      ...
      if (request.method === "POST" && requestUrl.pathname === "/api/deletion-plans") {
        ...
        const id = plan.ids.length > 0 ? randomBytes(18).toString("base64url") : null;
        ...
        if (id) deletionPlans.set(id, savedPlan);
        const warnings = [];
        ...
        sendJson(response, 200, {
          plan: {
            ...
            expiresAtMs,
            id,
            skippedProtectionCount: prepared.skippedProtections.length,
