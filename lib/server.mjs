...
const ALLOWED_SORTS = new Set(["updated", "created", "name", "cwd"]);
...
function getRecords({ includeInternals, records, search, sort }) {
  return filterAndSortSessions({
    includeInternals: includeInternals === "true",
    records,
    search: search || "",
    sort: ALLOWED_SORTS.has(sort) ? sort : "updated",
  });
}
...
export async function startLocalServer({ codexHome, port = 0 }) {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "GET" && requestUrl.pathname === "/api/compatibility") {
        ...
        sendJson(response, 200, { ...diagnostic, currentVersions: await getInstalledProductVersions() });
        return;
      }

      if (request.method === "GET" && requestUrl.pathname === "/api/sessions") {
        const store = await loadSessionStore({ codexHome });
        const records = getRecords({
          includeInternals: requestUrl.searchParams.get("includeInternals"),
          records: store.records,
          search: requestUrl.searchParams.get("search"),
          sort: requestUrl.searchParams.get("sort"),
        });
        sendJson(response, 200, { records: records.map(formatSessionForJson) });
        return;
      }
      ...
      if (request.method === "POST" && requestUrl.pathname === "/api/deletion-plans") {
        ...
        const store = await loadSessionStore({ codexHome });
        const plan = planSessionDeletion({ recordIds: ids, store });
      ...
      if (request.method === "POST" && requestUrl.pathname === "/api/deletions") {
        ...
        if (scope === "deep") {
          ...
          if (diagnostic.status === "update-needed") {
            throw new Error("Deep cleanup is paused because this local storage layout needs an update.");
          }
        }
        ...
        try {
          ...
          const statusCode = verification.complete ? 200 : 500;
          sendJson(response, statusCode, { result, verification });
        } finally {
